...
def invariant_ids(reg):
 ids=[x.get("id") for x in reg.get("invariants",[]) if isinstance(x,dict)]; exp=[f"INV-{i:03d}" for i in range(1,51)]
 if ids!=exp: raise ValueError("architecture registry must contain exact ordered INV-001..INV-050")
 return ids
