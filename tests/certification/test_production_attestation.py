...
from aodsl.certification.attestation import create_attestation,verify_attestation
ROOT=Path(__file__).resolve().parents[2]
def setup(tmp):
 w=tmp/"repo"; shutil.copytree(ROOT,w,ignore=shutil.ignore_patterns(".git","dist","__pycache__",".pytest_cache"))
 e=w/"certification/evidence/live-certification-status.json"; m,a=create_attestation(w,e); mp=w/"certification/production-certification-manifest.json"; ap=w/"certification/production-certification-attestation.json"; mp.write_text(json.dumps(m)); ap.write_text(json.dumps(a)); return w,e,mp,ap
